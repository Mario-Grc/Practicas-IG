...
## Descripción
Lo primero que se añadió fueron las 3 franjas de colores, para crear la bandera simple. Esto se hizo dividiendo el espacio vertical en tres sectores, haciendo el central el doble de ancho como en la bandera oficial.
```
    vec3 color;
    if (uv.y < 0.25) {
...
Lo siguiente fue añadir el efecto de ondulación, que se consiguió generando una onda sinusoidal. Esta onda tiene su `frecuency`, `amplitude` y su `speed` para poder modificarla fácilmente. Como quedaba muy simple, también añadí la línea `wave *= st.x;`, que añade mucho realismo haciendo que la bandera ondee más en cuando más alejada del margen izquierdo está.
...
El resultado final está en el archivo [espana.frag](code/espana.frag), un shader que usa técnicas matemátias para mostrar la bandera de España con un efecto que hace parecer que está ondeando al viento.
